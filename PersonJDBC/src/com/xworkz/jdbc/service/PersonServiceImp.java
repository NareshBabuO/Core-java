...
public class PersonServiceImp implements PersonService {
			...
			if (name != null && name.length() > 3 && name.length() < 15) {
				...
				if (email != null && email.length() > 10 && email.length() < 30) {
					...
					if (mobileNo > 50000 && mobileNo < 999999999) {
						...
						if (gender != null && gender.length() > 2 && gender.length() < 15) {
																	...
																	if (country != null && country.length() > 3
																		...
																		if (adharNo > 1000 && adharNo < 12999999) {
																			...
																			if (bankNo > 2000 && bankNo < 1000000000) {
																				...
																				pr.Save(dto);
																				pr.display();
																				pr.displayAllAgeGreaterThenOrderByName(
																						age);
																				pr.displayAllByGenderDescOrderByName(
																						gender);
																				pr.displayAllBySalaryGreaterThanOrderByDesc(
																						salary);
																				pr.displayCount();
	...
	public void display() {
		...
		PersonRepositoryImp pr = new PersonRepositoryImp();
		pr.display();
	}
